 ...
 * - Par défaut : sidebar visible uniquement sur desktop (cachée sur mobile).
 * - `embedded` : contenu seul (sans carte) pour l'afficher dans un tiroir mobile.
 */
export default function ProductFilters({ filters, onChange, onReset, embedded = false }) {
  ...
  // Contenu seul (dans le tiroir mobile)
  if (embedded) return <div className="px-5 py-4">{content}</div>;

  // Sidebar desktop uniquement
  return (
    <aside className="hidden lg:sticky lg:top-24 lg:block lg:self-start">
      <div className="rounded-2xl bg-white p-5 ring-1 ring-line dark:bg-primary-800 dark:ring-white/10">{content}</div>
    </aside>
  );
}
