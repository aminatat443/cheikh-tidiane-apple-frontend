...
export default function Navbar() {
  ...
  const CountIcon = ({ to, icon: Icon, count, label }) => (
    <Link to={to} className={iconBtnCls} aria-label={label}>
      <Icon size={19} />
      {count > 0 && (
  ...
  return (
    <header className="sticky top-0 z-40 text-white">
      {/* Barre unique — Noir profond #111827 */}
      <div className="border-b border-white/10 bg-primary">
        <div className="container-page flex h-16 items-center gap-3">
          {/* Logo (gauche) */}
          <BrandLogo />

          {/* Liens : centrés au repos, poussés (en flux) quand la recherche est ouverte */}
          <nav
            className={cn(
              'hidden h-16 items-center gap-1 lg:flex',
              searchOpen ? 'ml-2' : 'absolute left-1/2 -translate-x-1/2'
            )}
          >
            {navLinks.map((l) => (
              <Link
          ...
          {/* Barre de recherche en ligne : occupe l'espace et pousse les icônes */}
          {searchOpen && (
          ...
          <div className="ml-auto flex shrink-0 items-center gap-0.5">
            ...
            >
              {searchOpen ? <FiX size={20} /> : <FiSearch size={19} />}
            </button>
            <button
              onClick={toggleTheme}
              className={iconBtnCls}
              ...
              <div className="group relative ml-1">
                ...
                <div className="invisible absolute right-0 top-full w-56 origin-top-right translate-y-1 rounded-2xl border border-line bg-white p-2 text-primary opacity-0 shadow-card-hover transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 dark:border-white/10 dark:bg-primary-800 dark:text-white">
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-danger hover:bg-surface dark:hover:bg-white/5"
                  >
            ...
            <button
              className={cn(iconBtnCls, 'ml-1 lg:hidden')}
      ...
      {/* Menu mobile / tablette (noir profond) */}
      {mobileOpen && !searchOpen && (
        <div className="animate-fade-in border-b border-white/10 bg-primary lg:hidden">
              ...
              {navLinks.map((l) => (
                <Link
                  ...
                  className={cn(
                    ...
                    isLinkActive(l.to) ? 'bg-white/5 text-accent' : 'text-white/80 hover:bg-white/10 hover:text-white'
                  )}
                >
                  {l.label}
                </Link>
